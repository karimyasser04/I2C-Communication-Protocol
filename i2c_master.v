    ...
    case (state)
            ...
            WRITE_DATA: begin
                if (scl_fall) begin
                    ...
                    shift_reg <= {shift_reg[6:0], 1'b0};
                    if (bit_cnt == 0) begin
                        $display("Going to DATA_ACK at %0t", $time);
                        state <= DATA_ACK;
                        ack_released <= 1'b0;
                    end
                    else
...
always @(posedge clk) begin
    $display("T=%0t state=%0d bit=%0d scl=%b rise=%b fall=%b shift=%h",
        $time, state, bit_cnt, scl_in, scl_rise, scl_fall, shift_reg);
end
